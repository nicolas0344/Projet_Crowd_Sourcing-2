...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE {$\text{\textbf{ETAPE M :} \textit{Calculer les }} \left(\widehat{\pi^k_{l,j}}\right)_n = \displaystyle \frac{\displaystyle \sum_i \left(T_{i,l}\right)_{n-1} n^k_{i,j}}{\displaystyle \sum_j \displaystyle \sum_i \left(T_{i,l}\right)_{n-1} n^k_{i,j}} $, $\left(\widehat{p_l}\right)_n = \displaystyle \frac{\displaystyle \sum_i \left(T_{i,l}\right)_{n-1}}{I}$}
		\STATE {$\text{\textbf{ETAPE E :}\textit{ Calculer }} \left(\widehat{T_{i,l}}\right)_n = \displaystyle \frac{ \left(\widehat{p_l}\right)_n \displaystyle \prod_k^K \displaystyle \prod_j^J \left(\widehat{\pi^k_{l,j}}\right)_n^{n^k_{i,j}}}{\displaystyle \sum_q^J \left(\widehat{p_q}\right)_n \displaystyle \prod_k^K \displaystyle \prod_j^J \left(\widehat{\pi^k_{l,j}}\right)_n^{n^k_{i,j}}}$;}
		\ENDFOR
