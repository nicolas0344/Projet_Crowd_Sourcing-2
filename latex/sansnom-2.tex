...
\begin{itemize}[label=\adfflowerleft]
	...
	\item On dispose egalement d'un esemble de médecin $\{1, \cdots, K\}$, et on considère $\forall i \in \{1, \cdots, I\}$ le vecteur aléatoire $(Y^k_i)_{k \in \{1, \cdots, K\}}$ le diagnostique du médecin $k$ au patient $i$, $Y^k_i$ à valeur dans $\{1, \cdots, J\}$. 
\end{itemize}
...
\begin{hyp}
	Nous supposons que, $\forall j,k \in \llbracket 1,J \rrbracket \times \llbracket 1,K \rrbracket$, $Y^k_i | X_i = \alpha_i \sim Multinomiale((\pi^k_{\alpha_i,j}),1)$
\end{hyp}
